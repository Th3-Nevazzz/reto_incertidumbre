...
Urban mobility in Mexico is shaped by socioeconomic, demographic, and contextual factors that affect both transport mode choice and travel time. We apply multinomial Bayesian networks to the 2017 Origin–Destination Survey (INEGI) to model these dependencies under uncertainty. We specified three candidate directed acyclic graphs (DAGs) and fitted them by maximum likelihood, comparing model fit with BIC/AIC. DAG 3 achieved the best fit (BIC = −1,094,177; AIC = −985,745.9), and we use it as the reference model to answer four policy-relevant queries: (i) automobile use by men given religious travel purpose, (ii) the probability that trips longer than 60 minutes are motorized versus non-motorized, (iii) the probability that home-to-school trips exceed 60 minutes when school transport is used, and (iv) the likelihood of public transport use (bus, light rail, suburban rail) among high-stratum individuals in Cuauhtémoc borough on Saturdays.

Results indicate that the probability of automobile use by men attending religious trips is **20.6%** (Query 1). For long trips (greater than 60 minutes), motorized transport strongly dominates: the conditional probability of using a motorized mode is **80.9%**, compared to **19.3%** for non-motorized (Query 2). The probability that home-to-school trips last more than 60 minutes when using school transport is very low (**1.9%**) (Query 3). Finally, the likelihood of high-stratum individuals in Cuauhtémoc borough using public transport on Saturdays is also low (**2.1%**) (Query 4). These findings highlight the usefulness of Bayesian networks to capture interdependencies in travel behavior and to inform evidence-based urban mobility planning.

**Keywords:** Bayesian networks, transport, multinomial models, DAG, urban mobility, Mexico
