...
```
```{r}
datos <- droplevels(na.omit(data))

bn_dag1 <- bn.fit(dag, data = datos, method = "mle")
bn_dag1
```
```{r}
arc_strength_mi <- arc.strength(dag, data = datos, criterion = "mi")
arc_strength_mi <- arc_strength_mi[order(-arc_strength_mi$strength), ]
kable(arc_strength_mi, caption = "Arc strength (Mutual Information)")

arc_strength_bic <- arc.strength(dag, data = datos, criterion = "bic")
arc_strength_bic <- arc_strength_bic[order(arc_strength_bic$strength), ]
kable(arc_strength_bic, caption = "Arc scores (BIC)")
```
